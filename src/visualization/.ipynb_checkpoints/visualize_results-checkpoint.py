...
# Add parent directory for module imports if running from src/
...
class ResultVisualizer:
    def __init__(self, config, checkpoint_path):
        ...
        # Class names and colors (edit as appropriate for your dataset)
    ...
    def visualize_case(self, case_id, max_slices=16, save_path=None):
        case_slices = [i for i, (cid, _) in enumerate(self.val_dataset.slices) if cid == case_id]
        if not case_slices:
...
def main():
    parser = argparse.ArgumentParser(description='Visualize segmentation results (U-Net/Attention U-Net)')
